...
############################################################
#Feature 3: busiest hours for the last 60 minutes
def BusiestHoursLast(f1, h,last60min):
    dt=h[3].str.strip('[')
    dt=pd.to_datetime(dt, format="%d/%b/%Y:%X")
    dt=dt.to_frame()
    dt.set_index(pd.DatetimeIndex(dt[3]),inplace=True)
...
HOURS60_FILE='../log_output/hours60.txt'

CHUNKSIZE=100000
t = time.time()

#####For Calculation of Computation Time
tHost=0
tResources=0
tHours=0
tBlocked=0
...
for df in pd.read_csv(LOG_FILE,sep=' ', header = None,chunksize=CHUNKSIZE,parse_dates=[3,4],infer_datetime_format=True
    ...
    ####OPTIONAL FEATURE FIVE  - LIKE FEATURE 3 WITH NO OVERLAPING (Round to the nearest hour)
    #Hours60=BusiestHoursBis(Hours60,df)
...
Hours, last60min=BusiestHoursLast(Hours,tmp, last60min)
...
"""
####FEATURE 5###########
Hours60['idx']=Hours60.index
Hours60=Hours60.sort_values(by=[3,'idx'],ascending=[False,True])
Hours60.idx=Hours60.idx.apply(lambda x: x.strftime("%d/%b/%Y:%X -0400"))
Hours60=Hours60[['idx',3]]
Hours60[3]=Hours60[3].astype(int)
Hours60.to_csv(HOURS60_FILE,sep=',',header=None, index=False)    
"""
